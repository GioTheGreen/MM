...
Accepter::Accepter(Queue<std::string>& q, List<std::shared_ptr<sf::TcpSocket>>& s,Queue<myMessage>& myM ):
    queue_(q),
    socket_(s),
    Mqueue_(myM)
{}
...
void Accepter::operator()()
{
    ...
    while (true)
    {
        ...
        {
            socket_.push(socket);
            std::shared_ptr<Receiver> receiver = std::make_shared<Receiver>( socket, queue_, Mqueue_);
