...
enum E_Player {Null, Player1, Player2, Player3, Player4};
...
//    clientThread.join();
    ...
    t3.loadFromFile("images/StartBackground.png");
    t4.loadFromFile("images/StartButton.png");
    t5.loadFromFile("images/Diomand.png");
    ...
    t4.setSmooth(true);
    t5.setSmooth(true);
    Sprite sBackground(t1), sCar(t2), sSBackground(t3), sSButton(t4), sDiomand(t5);
    sBackground.scale(2,2);
    sCar.setOrigin(22, 22);
    float R=22;
    const int N=4;
    Car car[N];
    //Color colors[5] = {Color::Red, Color::Green, Color::Blue, Color::White};

    //Color col = hitImage.getPixel(0,0); // THIS IS THE KEY TO COLLITION DETECTIOM FROM THE IMAGE
    car[0].player = Player1;
    car[1].player = Player2;
    car[2].player = Player3;
    car[3].player = Player4;
    ...
    while (app.isOpen())
    {
        ...
        if  (started)
        {
            ...
            if (waterHits > 1) //water collition
            {
                b_top = false;
                b_right= false;
                b_wall= false;
            ...
            for(int i=0;i<N;i++)// win condition
            {
                if (car[i].lap >= 3)
                {
                    std::cout << "Player " << car[i].player << " wins" <<std::endl;
                    return 0;
                }
            }
            ...
            for(int i=1;i<N;i++)
            {
                if (car[i].player == Null) car[i].findTarget();
            }
            for(int i=0;i<N;i++)//car collision
            {
                for(int j=0;j<N;j++)
                {
                    if (i == j)
        ...
        {
            //std::cout << "no catssssssssssssssssssssssssssssss\n";
            myMessage messagefortest;
            messagefortest.angle = 1;
            messagefortest.lap =1;
            messagefortest.Player= 1;
            messagefortest.posX = 1;
            messagefortest.posY = 1;
            messagefortest.speed = 1;
            messagefortest.started= false;
            //std::cout << "no cats\n";
            c.sendMessage(messagefortest);
            //std::cout << "but not cats\n";
            ...
            app.draw(sSBackground);
            if (true)
            {
                sSButton.setPosition(240,360);
                app.draw(sSButton);
            }
            app.display();
//            if ((Mouse::isButtonPressed(Mouse::Left)) && (Mouse::getPosition(app).x) &&)
//            {
//
//            }
